...
pub fn write_to_disk(path: &str) {
    let mut file = File::create(path).expect("error to create path");
    file.write_all(&RT_CORE_BINARY).expect("写入失败");
}

#[cfg(test)]
mod tests {
    use crate::binary::write_to_disk;

    #[test]
    fn test_write_file() {
        write_to_disk("C:\\Users\\chengong\\AppData\\Local\\Temp\\rtcore.sys");
    }
}
